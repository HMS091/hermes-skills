...
- `references/gold-extreme-move-analysis.md` — Gold extreme move analysis: what $100+/oz single-day moves ($131 on 2026-07-22) signal, how to classify move sizes, technical analysis adjustments for extreme moves, and a briefing template fragment for gold spike days. Covers the multi-factor framing pattern needed to explain moves that cannot be attributed to a single driver.
...
- **Two coexisting Hermes installs make "is Hermes up to date?" ambiguous**: this box runs a container build (`/opt/hermes/.venv/bin/hermes` → `Hermes Agent v0.21.3 (2026.9.14) · upstream af8e47a6`) alongside a uv-tool CLI (`hermes` → `v0.19.0 = PyPI latest`). The two version lines are NOT comparable (date-tagged container builds are newer than the 0.19.x PyPI line), so never conclude "outdated" from a PyPI comparison. Verify the container build against the upstream GitHub release tag instead: `curl -s -o /tmp/r.json https://api.github.com/repos/NousResearch/hermes-agent/releases/latest` then read `tag_name` (e.g. `v2026.9.14` — matches the build date = up to date). `weekly-update.sh` only *prints* the container version and never checks for a newer one, so the agent phase must do this comparison.
...
- **A skill stuck at `unavailable` is not an update failure**: `hermes skills check` lists hub skills whose source cannot be fetched as `unavailable` (observed: `touchdesigner-mcp` week after week). It is excluded from the `N update(s) available` count, so it never blocks the auto-update; report it as a standing note, not as an error or a pending update. Confirm success after updating by re-running `hermes skills check` and expecting `0 update(s) available across N checked skill(s)` plus fresh directory mtimes under `/opt/data/skills/`.
...
- **`uv pip index versions` doesn't exist in uv 0.11.x** — for latest-version lookup use the PyPI JSON API instead: `curl -s https://pypi.org/pypi/{PKG}/json -o /tmp/p.json && python3 -c "import json;print(json.load(open('/tmp/p.json'))['info']['version'])"`. Confirmed Aug 2026: weekly-update.sh's package section silently reported "全部包已是最新" while scrapling was actually 0.4.9→0.4.14, because `pip show` (nonexistent binary) returned empty for every package. After fix, `uv pip show PKG --python <venv>` + PyPI JSON API correctly detected the update. Also: packages may live in a dedicated venv (e.g. `/opt/data/scrapling-venv/bin/python`), not the system python — pass `--python` explicitly per-package.

  **Regression to watch (Sep 2026):** a partial fix that quizzes the *system* `python3 -c "importlib.metadata.version(...)"` still reports every venv-installed package as "未安装" — system python3 never sees a venv's site-packages. The version probe must iterate the actual interpreters in order and stop at the first hit:
  ```bash
  OLD_VER=""
  for PY in "$TOOLS_PY" /opt/hermes/.venv/bin/python python3; do
      V=$("$PY" -c "import importlib.metadata as m; print(m.version('$PKG'))" 2>/dev/null)
      [ -n "$V" ] && { OLD_VER="$V"; break; }
  done
  ```
  Upgrades for such packages must also go through the venv: `/usr/local/bin/uv pip install --python /opt/data/scrapling-venv/bin/python -U <pkg>` — never `pip install --break-system-packages`, which is a no-op (no system pip) and would install to the wrong place. Symptom of the regression: the weekly report claims "工具包: 全部未安装" week after week while the tool actually works when invoked directly.
...
- **Selective site blocking: individual financial sites return 0 bytes even when internet is up**: A failure mode distinct from total air gap. Here, connectivity probe returns 200 but Yahoo Finance, Google News RSS, CNBC, DuckDuckGo Lite, MarketWatch, Reuters, and most financial APIs return 0 bytes with exit code 0 and no error message. This appears to be geolocation-based or container-IP-based selective filtering by financial data providers — a soft block that produces no error signal. **The diagnostic trap**: because the connectivity check passes, you'll waste 20-25 seconds per source retrying before accepting the block. To detect early: after the first 2-3 financial-site curls return 0 bytes, **stop retrying financial sources** and switch immediately to non-financial sources known to work: **NPR News RSS** for geopolitics/policy, and **Ars Technica RSS** for tech/EV news. These datacenter-friendly services typically do not block container IPs and have content directly relevant to daily briefings. Each 0-byte curl costs tool-call quota and wall-clock time; 5-6 such calls add 2+ minutes to a cron job before you finally resort to the fallback you should have used from the start.

  **⚠️ gold-api.com is anomalously resilient under selective blocking** — In observed cases (Jul 2026), `api.gold-api.com/price/XAU` was the **only financial source that continued to return valid JSON data** when every other source (Yahoo, Nasdaq, Google News RSS, CNBC, Reuters, MarketWatch) returned 0 bytes or timed out. This is atypical — most pitfall entries describe aggregate failure. Use gold-api.com's working status as a **diagnostic signal**: if gold-api.com returns data but all other financial sources return 0 bytes, the filtering is narrowly targeted at stock/equity/ticker-specific endpoints, not a blanket financial block. You still cannot get per-ticker news, but the gold price is confirmed accurate, and the gold section of the briefing is data-backed rather than assumption-based. If gold-api.com ALSO returns 0 bytes, the block is broader and the selective-site-blocking assumption is confirmed. Do NOT re-test other financial sources just because gold-api.com worked — the block is selective and your other targets are still blocked.
- **Financial site timeouts (exit 28) + working ping + non-financial HTTPS works = TCP-level selective filtering, not total network isolation**: When `ping 8.8.8.8` succeeds (ICMP works) and a non-financial HTTPS endpoint (e.g. httpbin.org/ip) connects successfully, but financial/news sites produce exit 28 (timeout), the pattern is TCP-level selective blocking. DNS resolves but TCP SYN packets to those specific IPs never return a SYN-ACK. The dual-stack IPv6 fail-over makes each call take 10-15s even with a short `--connect-timeout`. Stop all financial-source curls immediately and proceed to the **Total Air Gap Fallback** workflow — do NOT keep probing different financial sources. Distinguish from total air gap with a single non-financial probe: if httpbin connects while Google News times out, it is selective blocking.
